...
bool File::Open(const char* file_path, uint32_t flags)
{
	...
	m_fid = open(file_path, flags, 0644);
	umask(old_mask);

	return (m_fid != INVALID_FILEID);
}

void File::Close()
{
    if(m_fid != INVALID_FILEID)
    {
        close(m_fid);
        m_fid = INVALID_FILEID;
    }
...
{
    ssize_t rs = read(m_fid, buf, buf_size);
    while(rs == -1 && LastError == EINTR)
    {
        rs = read(m_fid, buf, buf_size);
    }
    return rs;
}
...
{
    ssize_t rs = pread(m_fid, buf, buf_size, offset);
    while(rs == -1 && LastError == EINTR)
    {
        rs = pread(m_fid, buf, buf_size, offset);
    }
    return rs;
}
...
{
    ssize_t ws = write(m_fid, buf, buf_size);
    while(ws == -1 && LastError == EINTR)
    {
        ws = write(m_fid, buf, buf_size);
    }
    return ws;
}
...
{
    ssize_t ws = pwrite(m_fid, buf, buf_size, offset);
    while(ws == -1 && LastError == EINTR)
    {
        ws = pwrite(m_fid, buf, buf_size, offset);
    }
    return ws;
}
